...
public abstract class L2Character extends L2Object implements ISkillsHolder
{
	...
	public boolean doDie(L2Character killer)
	{
		...
		if ((killer != null) && (killer.isPlayer() || killer.isSummon()))
		{
			if (isRaid())
			{
				if (killer.getActingPlayer().isInParty())
				{
					for (L2PcInstance o : killer.getActingPlayer().getParty().getMembers())
					{
						if (o != null)
						{
							o.getCounters().onRaidKill();
						}
					}
				}
				else
				{
					killer.getActingPlayer().getCounters().onRaidKill();
				}
			}
			else if (isMonster())
			{
				killer.getActingPlayer().getCounters().onMobKill();
			}
			else if (isSummon())
			{
				if ((killer.getActingPlayer().getSummon() != null) && (killer.getActingPlayer().getSummon().getObjectId() == getObjectId()))
				{
					return false;
				}
				killer.getActingPlayer().getCounters().onSummonKill();
			}
		}
